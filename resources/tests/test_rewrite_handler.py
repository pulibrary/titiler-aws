...
def test_rewrites_mosaic_id_production():
    ...
    assert output['querystring'] == 'url=https%3A%2F%2Fmap-tiles.princeton.edu%2Fresources%2Fbanana'
...
def test_rewrites_mosaic_id_staging():
    event = {'Records': [{'cf': {'request': {'uri': 'https://test.com/mosaicjson', 'querystring': 'id=banana'}}}]}
    output = handler_staging(event, {})
    assert output['querystring'] == 'url=https%3A%2F%2Fmap-tiles-staging.princeton.edu%2Fresources%2Fbanana'
